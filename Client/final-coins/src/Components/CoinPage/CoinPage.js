...
class CoinPage extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`/coinPageById/${id}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({data:json})
        })
    if (this.props.location.state) {
      const coin_id = this.props.location.state.id;
      const username = window.localStorage.getItem("login");
      const role = window.localStorage.getItem("role");
      if(role==='User' && coin_id!=id) {
        fetch(`/history/${id}`, {
          method: "POST",
          body: JSON.stringify({
            username: username,
           ...
           <Link
               to={{
                 ...
                 state: {
                   ...
                   searchHistory:this.props.location.state?this.props.location.state.searched:false
                 },
               }}
           >
             Back to the list
